...
### 🐛 Bug 修复
- **修复自动主题切换在定时推送时不生效**：模板缓存使用固定键 `main_template` 不区分主题，导致白天首次生成浅色排行榜后缓存了浅色模板，晚上定时推送命中缓存返回浅色模板。现在缓存键包含模板文件名（如 `main_template:rank_template_cartoon_dark`），确保深浅色主题各自独立缓存，切换时段后正确加载对应模板。
